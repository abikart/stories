...
- Every active route, player surface, and drag-to-guide element now has one
  documented outcome: solid, Light Glass, atmospheric glow, spotlight glow,
  static sheen, animated sheen, or no treatment. A negative decision is durable
  design-system data, not unfinished styling.
- Light Glass is the only exact-source effect and remains stable. Glow and sheen
  are labeled source-guided and provisional until their detailed Figma recipe
  frames can be audited. The stable solid surface is explicitly Stories-native
  and does not claim Figma pixel parity.
- The silver-path goal is the only production Spotlight Glow. It maps the local
  single-target rule to Fern's authored gold palette while retaining a visible
  goal ring and independent keyboard focus. The interaction layer's former
  full-stage tint was removed so it cannot masquerade as atmosphere.
- Catalog and story QA now run in Chromium and WebKit, including the explicit
  no-backdrop-filter fixture, reduced motion, guide completion, touch-safe
  geometry, alpha fallback, reverse settlement, and six responsive presets.
- Before/after evidence is versioned beside the design-system documentation and
  regenerated by a repository script. This is the release proof for the surface
  hierarchy, not a mandate to freeze feature composition.
