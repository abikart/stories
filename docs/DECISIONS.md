...
- The approved POC is frozen at local tag `poc-success`; the MVP continues on
  `codex/mvp-immersive-runtime` without pushing.
- V1 routes, engine, content, render scripts, Pip regression content, completed
  proof routes, and superseded POC documents are retired. Git history is their
  source of truth if a technique needs to be recovered.
- Durable v2 documents are promoted into `docs/product`, `docs/research`, and
  `docs/mvp`; `docs/README.md` is the sole documentation index.
- The root route is a minimal entrance to Fern. `/experience/[storyId]`, the
  byte-range content route, and `/dev/viewport` are the only active surfaces.
